// UnitLAB4 — 🧪 Python Lab · Experiment 4 (e-observation record)
// Inheritance & Polymorphism (CO2 · Sessions 7–8).
//
...
//                          brick (e.g. "UnitLAB4@p1_algo") to the Events/
...
//
// EXPERIMENT-4 NOTE. Program 1 (basic inheritance) is driven by literal
// values, so its hidden test uses a `transform` (find/replace pairs) to
// swap the staff member — a hard-coded print then fails while genuine
// inherited behaviour still passes. Program 2 (method overriding &
// polymorphism) has its calls fixed structurally — print(d.role()) and
// print(n.role()) — leaving only the method NAME as a blank; its hidden
// `transform` edits the method bodies so the printed text can only be
// right if the objects truly dispatch to their own overridden role().
// Program spec is taken verbatim from the MED23CL202 lab manual,
// Experiment 4 (pages 12–15).
...
const UNIT_ID = "UnitLAB4";
const mono = { fontFamily: "monospace", fontSize: 12.5, color: C.text, margin: 0, lineHeight: 1.8, whiteSpace: "pre-wrap" };

// ─────────────────────────────────────────────────────────────
//  EXPERIMENT DATA — everything specific to Experiment 4 lives here.
// ─────────────────────────────────────────────────────────────
const PROGRAMS = [
  {
    key: "p1",
    title: "Program 1 — Basic Inheritance",
    aim: "Define a base Staff class with name, dept and a show() method, then create a Nurse subclass that inherits everything without rewriting it.",

    algo: [
      "Define Staff with __init__(self, name, dept) and show().",
      "Define Nurse(Staff) with no extra code — it inherits everything.",
      "Create a Nurse object and call show().",
    ],
    algoShuffle: [2, 0, 1],
    algoHint: "The parent class must be fully defined before a child can inherit from it; and the object can only be created after its class exists.",

    flow: [
      { type: "terminator", text: "Start" },
      { type: "process", text: "Define base class Staff — __init__ stores name, dept; show() prints them" },
      { type: "process", text: "Define Nurse(Staff) — inherits everything, no new code" },
      { type: "process", text: 'Create  n = Nurse("Anita", "ICU")' },
      { type: "process", text: "Call  n.show()  (runs the inherited method)" },
      { type: "output", text: "Print:  Name: Anita | Dept: ICU" },
      { type: "terminator", text: "Stop" },
    ],
    flowShuffle: [3, 1, 5, 0, 2, 6, 4],
    flowHint: "Start and Stop are the rounded caps. Staff must be defined first, then Nurse can inherit from it, then a Nurse object can be created and its inherited show() called; printing is the last real step.",
    flowNote: "There is no diamond here: Nurse writes no code of its own. Creating the Nurse object silently runs Staff's __init__, and n.show() runs Staff's show() — both reused through inheritance.",

    codeLines: [
      [{ text: "class Staff:" }],
      [{ text: "    def __init__(self, name, dept):" }],
      [{ text: "        self.name = name" }],
      [{ text: "        self.dept = dept" }],
      [{ text: "    def show(self):" }],
      [{ text: '        print("Name:", self.name, "| Dept:", self.dept)' }],
      [{ text: "class Nurse(" }, { blank: 0 }, { text: "):" }, { cmt: "inherit from Staff" }],
      [{ text: "    pass" }],
      [{ text: "n = " }, { blank: 1 }, { text: '("Anita", "ICU")' }, { cmt: "create a Nurse object (uses Staff.__init__)" }],
      [{ text: "n.show()" }],
    ],
    blankWidth: [58, 64],
    tests: [
      { inputs: [], expect: "Name: Anita | Dept: ICU", label: 'Nurse("Anita", "ICU") → Name: Anita | Dept: ICU' },
      // Hidden: swap the staff member's literals so a hard-coded print fails
      // while genuinely-inherited show() still prints the right values.
      { inputs: [], transform: [["Anita", "Kumar"], ["ICU", "ER"]], expect: "Name: Kumar | Dept: ER", label: "hidden staff → their details", hidden: true },
    ],
    progHints: [
      "A subclass inherits by naming its parent in parentheses: class Child(Parent). The parent here is Staff.",
      "You create the object by calling the subclass itself. So the two blanks are: Staff and Nurse.",
    ],
  },

  {
    key: "p2",
    title: "Program 2 — Method Overriding & Polymorphism",
    aim: "Give Doctor and Nurse their own version of a role() method defined in the base Staff class, so the same method name behaves differently per class.",

    algo: [
      "Define Staff with a role() method returning a generic description.",
      "Define Doctor(Staff) overriding role() with its own description.",
      "Define Nurse(Staff) overriding role() with its own description.",
      "Create one of each and print role() for both.",
    ],
    algoShuffle: [2, 3, 0, 1],
    algoHint: "The base class must exist before either subclass can extend it; the objects can only be created and printed once all three classes are defined.",

    // The polymorphism itself is the decision diamond: the SAME call role()
    // resolves to a different body depending on the object's actual class.
    flow: [
      { type: "terminator", text: "Start" },
      { type: "process", text: "Define base Staff with role() (generic description)" },
      { type: "process", text: "Define Doctor(Staff) overriding role()" },
      { type: "process", text: "Define Nurse(Staff) overriding role()" },
      { type: "decision", text: "Object's actual class? (Doctor / Nurse)" },
      { type: "process", text: "d = Doctor(); n = Nurse(); call role() on each" },
      { type: "output", text: "Print each staff member's own role" },
      { type: "terminator", text: "Stop" },
    ],
    flowShuffle: [3, 0, 5, 1, 6, 2, 4, 7],
    flowHint: "Define the base first, then each subclass override, then create the objects. The diamond isn't an if you wrote — it's Python choosing which role() to run based on the object's class.",
    flowNote: "The diamond captures POLYMORPHISM: you write the same call, role(), for both objects, and Python picks Doctor's version for the Doctor and Nurse's version for the Nurse. That is 'many forms' from one method name.",

    codeLines: [
      [{ text: "class Staff:" }],
      [{ text: "    def role(self):" }],
      [{ text: '        return "General staff member"' }],
      [{ text: "class Doctor(Staff):" }],
      [{ text: "    def role(self):" }],
      [{ text: '        return "Doctor - diagnoses & treats patients"' }],
      [{ text: "class Nurse(Staff):" }],
      [{ text: "    def " }, { blank: 0 }, { text: "(self):" }, { cmt: "override the same method name" }],
      [{ text: '        return "Nurse - monitors & assists patients"' }],
      [{ text: "d = Doctor()" }],
      [{ text: "n = Nurse()" }],
      [{ text: "print(d.role())" }],
      [{ text: "print(n." }, { blank: 1 }, { text: "())" }, { cmt: "call the overridden method on the Nurse" }],
    ],
    blankWidth: [58, 58],
    tests: [
      { inputs: [], expect: "Doctor - diagnoses & treats patients\nNurse - monitors & assists patients", label: "Doctor & Nurse → each prints its own role" },
      // Hidden: edit the method BODIES. The printed text can only match if
      // each object truly dispatches to its own overridden role() — proving
      // real polymorphism rather than an echoed literal.
      { inputs: [], transform: [["diagnoses & treats", "diagnoses and treats"], ["monitors & assists", "monitors and assists"]], expect: "Doctor - diagnoses and treats patients\nNurse - monitors and assists patients", label: "hidden bodies → polymorphic dispatch", hidden: true },
    ],
    progHints: [
      "Overriding means re-using the exact same method name in the subclass. The method is called role, and you call it the same way on n as on d.",
      "Both blanks are the method name role — one defines the override inside Nurse, one calls it on n.",
    ],
  },
];
...
//  against a visible test and a hidden one. Hidden tests may carry a
//  `transform` (swap literals) and/or a `setup` (code prepended before
//  the program — used by file experiments to seed a file on disk).
// ─────────────────────────────────────────────────────────────
  ...
  return (
    <div>
      ...
      <p style={{ color: C.muted, fontSize: 13, marginBottom: 12, lineHeight: 1.7 }}>
        ...
        program is tested twice — the second run changes the values, so no shortcuts. ▶
      </p>
...
// ─────────────────────────────────────────────────────────────
export default function UnitLAB4({ student, onUnitComplete, challengeProgress = [], onStageComplete }) {
    ...
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "'Segoe UI', system-ui, sans-serif", color: C.text, paddingBottom: 40 }}>
      ...
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "24px 16px" }}>
        <div style={{ background: C.teal + "10", border: `1px solid ${C.teal}33`, borderRadius: 10, padding: "10px 16px", marginBottom: 18, fontSize: 12.5, color: C.muted, lineHeight: 1.7 }}>
          🧭 <strong style={{ color: C.teal }}>Aim.</strong> Reuse and extend behaviour across classes using inheritance,
          override methods to specialise behaviour, and use polymorphism so the same method call works differently for
          different hospital-staff objects. Each step you finish is saved to your e-observation record.
        </div>
