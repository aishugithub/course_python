// Unit6_C — 🔥 The Crucible: Module 6 (Repetition)
// Same gated 4-stage arc; Spark now mixes THREE challenge types:
// predict-the-output, trace-the-state, and the new spot-the-lie.
  ...
  const questions = [
    {
      ...
      answer: 1,
      hints: ["range starts at 0 unless you say otherwise.", "range(3) hands out numbers UP TO but NOT including 3."],
      why: "range(3) → 0, 1, 2. Starts at 0, stops just before 3 — the loop body runs 3 times.",
    },
    {
      kind: "trace",
      code: "total = 0\nfor i in range(1, 4):\n    total = total + i\nprint(total)",
      ...
      hints: ["range(1, 4) hands out 1, 2, 3 — not 4.", "Follow total through each pass: 0 → 0+1 → 1+2 → 3+3."],
      why: "The accumulator pattern: total goes 0 → 1 → 3 → 6 as i takes 1, 2, 3. Prints 6.",
    },
    {
      kind: "lie",
      code: "for i in range(2, 10, 2):\n    print(i)",
      claims: [
        "It prints 2, 4, 6, 8 — one per line.",
        "The loop body runs exactly 4 times.",
        "The last number it prints is 10.",
      ],
      lieIndex: 2,
      hints: ["Two of these claims agree with each other — the lie contradicts them.", "range STOPS BEFORE its second number. Does 10 ever get handed out?"],
      why: "range(2, 10, 2) → 2, 4, 6, 8 — the stop value 10 is never included. Claims 1 and 2 are true; claim 3 is the lie.",
    },
  ];
  ...
  return (
    <div>
      <p style={{ color: C.muted, fontSize: 13, marginBottom: 16, lineHeight: 1.7 }}>
        Warm-up, three different weapons: predict an output, trace an accumulator by hand, and — new this
        module — read three claims about a loop and <em>click the one that's lying</em>. ✨
      </p>
...
function FlameStage({ onPass }) {
  const bugs = [
    {
      intro: "This countdown should print 1 to 5 and stop — instead it runs FOREVER. Click the line that makes the loop immortal.",
      lines: ["count = 1", "while count <= 5:", "    print(count)", "    count = count - 1"],
      buggyLine: 3,
      lineHints: {
        0: "count starts at 1 — that's fine.",
        1: "The condition is a fair question. The problem is whether the answer ever CHANGES to False.",
        2: "The print is innocent — it just reports.",
      },
      fixes: ["count = count + 1", "count = 1", "print(count - 1)"],
      fixAnswer: 0,
      fixHints: ["For the loop to END, count must eventually pass 5. Which direction is it moving now?", "count goes 1 → 0 → -1 → … it's running AWAY from 5. Reverse it."],
      why: "count = count - 1 moves AWAY from the exit condition, so count <= 5 stays True forever. Every while needs a step towards its exit.",
    },
    {
      intro: "This should print the numbers 1 to 10 — it runs without crashing, but 10 never appears. Click the buggy line.",
      lines: ["for i in range(1, 10):", "    print(i)"],
      buggyLine: 0,
      lineHints: {
        1: "The print is innocent — it prints every number it's handed. The question is what it's handed.",
      },
      fixes: ["for i in range(1, 11):", "for i in range(0, 10):", "for i in range(10):"],
      fixAnswer: 0,
      fixHints: ["range stops one step BEFORE its second number.", "To reach 10, the stop value must be one past it."],
      why: "The classic off-by-one: range(1, 10) stops at 9. To include 10, stop at 11 — range's stop value is always excluded.",
    },
  ];
  ...
  return (
    <div>
      <p style={{ color: C.muted, fontSize: 13, marginBottom: 16, lineHeight: 1.7 }}>
        Two of the most famous loop bugs in programming: the infinite loop and the off-by-one. Find WHERE
        each lives, then choose the fix. Bug {Math.min(fixedCount + 1, bugs.length)} of {bugs.length}. 🔥
      </p>
...
function ForgeStage({ onPass }) {
  const target = [
    { code: 'n = int(input("Which table? "))', defines: "n", needs: [] },
    { code: 'print("Multiplication table of", n)', defines: "header", needs: ["n"] },
    { code: "for i in range(1, 11):", defines: "for_open", needs: ["n", "header"] },
    { code: "    row = n * i", defines: "row", needs: ["for_open"] },
    { code: '    print(n, "x", i, "=", row)', defines: null, needs: ["row"] },
  ];
  const MSG = {
    n: "the locker n doesn't exist yet — the input line must come first",
    header: "the title should print once, BEFORE the loop starts — otherwise it repeats or comes too late",
    for_open: "there's no for above it to belong to — indented lines need their loop",
    row: "row is computed inside the loop — this line must come after it",
  };
  const shuffled = [3, 1, 4, 0, 2]; // fixed shuffle so the puzzle is stable
  ...
  const check = () => {
    ...
    setVerdict({ ok: true, msg: "Ten rows, hammered out by a single loop. The multiplication-table printer works!" });
    setSolved(true);
  };
  ...
  return (
    <div>
      <p style={{ color: C.muted, fontSize: 13, marginBottom: 16, lineHeight: 1.7 }}>
        The forge takes scattered metal and hammers it into shape. Below is a multiplication-table printer —
        with its lines scrambled. Use ↑↓ to rebuild it. Watch the indentation: the indented lines live
        <em> inside</em> the loop and repeat ten times; the rest run once. ⚒️
      </p>
...
function TemperStage({ onPass }) {
  const TESTS = [
    { pre: "n = 5\n", expect: "15", label: "n = 5 → 15" },
    { pre: "n = 10\n", expect: "55", label: "hidden n → its sum" },
    { pre: "n = 1\n", expect: "1", label: "hidden edge case" },
  ];
  ...
  const hints = [
    "The accumulator pattern from 6.2: start a total at 0, add each number to it inside a loop.",
    "for i in range(1, n + 1): total = total + i — remember the stop value is excluded, hence n + 1. Then print(total).",
  ];
  ...
  async function run() {
    ...
    // The hidden tests supply `n` themselves (a different value each time). If the
    // learner also writes `n = ...`, their line clobbers ours and every test runs
    // on the same number — giving confusing expected/output mismatches. Catch it
    // up front.
    if (/(^|\n)\s*n\s*=(?!=)/.test(code)) {
      setStatus("failed");
      setMessage("Remove the line that sets  n = ...  from your code.\nThe value n is provided automatically and changes on each hidden test — if you redefine it, your code always runs on the same number.");
      return;
    }
    try {
      ...
      setStatus("passed");
      setMessage("All three tests passed — including the hidden edge case. That loop earns its keep.");
    } catch (e) {
  ...
  const fbTarget = [
    ...
    { code: "for i in range(1, n + 1):", defines: "for_open", needs: ["total"] },
    { code: "    total = total + i", defines: "acc", needs: ["for_open"] },
    { code: "print(total)", defines: null, needs: ["acc"] },
  ];
    ...
    <div>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.orange}55`, borderRadius: 10, padding: 14, marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.7 }}>
          A variable is already set for you: <code style={{ color: C.teal }}>n = 5</code>. Write code that
          prints <em>only</em> the sum 1 + 2 + … + n (for n = 5 that's 15). Your code is then re-run with
          different hidden values of n — so no hard-coding, use a loop!
        </div>
      </div>
...
function BadgeScreen({ claimed, onClaim }) {
  return (
    <div style={{ textAlign: "center", padding: 24 }}>
      ...
      <div style={{ fontSize: 26, fontWeight: 800, color: C.orange, marginTop: 8 }}>TEMPERED IN THE M6 CRUCIBLE</div>
      <div style={{ color: C.muted, fontSize: 14, marginTop: 10, lineHeight: 1.8, maxWidth: 480, margin: "10px auto 0" }}>
        Traced an accumulator by hand. Called out a lying claim. Slew an infinite loop AND an off-by-one.
        Then wrote a real loop that survived hidden tests. Module 6's repetition isn't just learned — it's{" "}
        <em>forged into skill</em>.
      </div>
      <div style={{
        ...
        <div style={{ color: C.text, fontWeight: 700, fontSize: 14, marginTop: 6 }}>Crucible Badge · Module 6</div>
      ...
      <div style={{ color: C.muted, fontSize: 13, marginTop: 18 }}>
        The <strong style={{ color: C.orange }}>M7 Crucible</strong> waits at the end of Organizing Data. 🔥
      </div>
      {!claimed && (
...
// ── Main ──
export default function Unit6_C({ student, onUnitComplete, challengeProgress = [], onStageComplete }) {
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "'Segoe UI', system-ui, sans-serif", color: C.text, paddingBottom: 40 }}>
      <div style={{ background: C.surface, borderBottom: `1px solid ${C.orange}44`, padding: "14px 24px", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div>
          <div style={{ fontSize: 12, color: C.orange, letterSpacing: 1, fontWeight: 700 }}>MODULE 6 › THE CRUCIBLE</div>
